...
const ClassroomView: React.FC<ClassroomViewProps> = ({ subject, design, onBack, onSelectConcept }) => {
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden flex flex-col font-['Fredoka']">
      {/* Header */}
      <div className="bg-white/95 backdrop-blur-md p-3 sm:p-5 flex justify-between items-center z-50 border-b-4 border-black/5 shadow-md">
      ...
      <div className="flex-1 relative transition-colors duration-1000" style={{ backgroundColor: design.wallColor }}>
        ...
        {showLyrics && activeMusic?.lyrics && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl text-center z-40 pointer-events-none">
            <div className="bg-white/90 backdrop-blur-xl p-8 sm:p-12 rounded-[3rem] sm:rounded-[5rem] shadow-2xl border-4 border-white animate-bounce-slow mx-6">
              <h3 className="text-3xl sm:text-6xl font-bold text-gray-800 transition-all duration-500 italic leading-tight">
        ...
        {/* Stickers Area */}
        <div className="absolute top-4 sm:top-8 w-full flex justify-center gap-8 sm:gap-16 flex-wrap px-12 sm:px-40 pointer-events-none z-10 h-[15%]">
          {design.posterUrls.map((url, i) => (
            <div key={i} className="w-16 h-16 sm:w-28 sm:h-28 animate-float-slow flex items-center justify-center pointer-events-auto" style={{ animationDelay: `${i * 0.7}s` }}>
        ...
        {design.shelves && design.shelves.length > 0 && (
          <div className="absolute top-[18%] sm:top-[22%] left-0 right-0 flex justify-center z-10">
            <div className="w-[85%] sm:w-[75%] h-3 sm:h-5 bg-black/5 rounded-full relative flex justify-around items-end px-6 sm:px-16">
               <div className="absolute inset-x-0 bottom-0 h-1 sm:h-1.5 bg-black/10 rounded-full"></div>
               {design.shelves.map((emoji, idx) => (
                 <span key={idx} className="text-xl sm:text-3xl mb-1 sm:mb-1.5 animate-wiggle hover:scale-150 transition-transform cursor-help" style={{ animationDelay: `${idx * 0.25}s` }}>
                   {emoji}
                 </span>
               ))}
            </div>
        ...
        <div className="absolute bottom-0 w-full h-[30%] border-t-8 border-black/5 shadow-[inset_0_10px_10px_rgba(0,0,0,0.05)] z-0" style={{ backgroundColor: design.floorColor }}>
          ...
          {design.mascot && design.mascot !== 'none' && (
            <div 
              ...
              className={`absolute bottom-6 sm:bottom-10 left-6 sm:left-16 text-6xl sm:text-9xl cursor-pointer transition-all z-30 ${isMascotCelebrating ? 'animate-celebrate scale-125 glow' : 'animate-bounce-pet'}`}
            >
              {MASCOTS.find(m => m.id === design.mascot)?.emoji}
              {isMascotCelebrating && <div className="absolute -top-8 -right-8 sm:-top-16 sm:-right-16 text-4xl sm:text-6xl animate-ping">✨</div>}
            </div>
          )}
        </div>
